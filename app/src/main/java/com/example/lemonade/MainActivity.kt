...
class MainActivity : AppCompatActivity() {
    ...
    private fun clickLemonImage() {
        /*
        Use this function to define the lemonade state upon clicking the image
         */
        when (lemonadeState) {
            // If the current state is "Select"
            SELECT -> {
                lemonadeState = SQUEEZE
                lemonSize = lemonTree.pick()
                squeezeCount = 0
            }
            // If the current state is "Squeeze"
            SQUEEZE -> {
                squeezeCount += 1
                lemonSize -= 1
                if (lemonSize == 0) {
        ...
        // Reset Image based on current lemonade state
        setViewElements()
    }
